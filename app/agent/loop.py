...
class LoopResult:
    ...
    spend: Decimal
    header: str
    # Phase 30: what normalize_for_model did on the served rung (e.g. "adaptive->enabled",
    # "effort-removed", "system-role-moved"), empty when nothing. On the per-request line.
    body_rewrites: list[str] = field(default_factory=list)
...
class _LoopState(TypedDict, total=False):
    ...
    # Phase 30: the body rewrite labels for the served rung, tracked next to the served
    # result so the log line reports the rung that was actually served.
    last_body_rewrites: list[str]
    best_body_rewrites: list[str]
    passed: bool
    stop_reason: str
...
async def _try_node(state: _LoopState) -> dict:
    ...
    # be circular. normalize_for_model is fully defined by the time a request runs the loop.
    from app.main import normalize_for_model

    ctx = state["ctx"]
    model = state["current_model"]
    payload = {**ctx["base_payload"], "model": model}
    # Phase 30: rewrite the body to fit this rung's own model right before the send, so an
    # escalation to an adaptive-only model (or a routed-down legacy rung) never 400s on a
    # field it does not support. The Anthropic adapter sends these raw bytes.
    body, body_rewrites = normalize_for_model(json.dumps(payload).encode(), model)
    ...
    updates: dict = {
        ...
        "last_result": result,
        "last_model": model,
        "last_body_rewrites": body_rewrites,
    }
    if result.status_code >= 400:
        ...
        updates["best_body_rewrites"] = body_rewrites
    return updates
...
async def _single_attempt(
    ...
    body, body_rewrites = normalize_for_model(json.dumps(swapped).encode(), model)
...
async def run_agent_loop(
    ...
    *,
    ...
    if best is not None:
        ...
        # The labels of the rung that is served (the last good answer in hand).
        body_rewrites = final.get("best_body_rewrites") or []
    else:
        ...
        body_rewrites = final.get("last_body_rewrites") or []
        if result is None:
